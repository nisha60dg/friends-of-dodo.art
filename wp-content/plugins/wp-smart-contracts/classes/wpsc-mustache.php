...
Any user that interacts with any blockchain using the plugin will be on their own and using their own technical resources. The website will offer reasonable help documentation as to how to use this plugin.<br /><br />
...
Using this plugin to deploy Smart Contracts in any blockchain, the user may have to pay a fee in cryptocurrencies. Also, the user will need to pay a gas fee for the transaction to be processed.<br /><br />
The deployment fee is a one-time payment to deploy Smart Contracts in the blockchain using the plugin. It doesn’t include any other service like support, design, or bug fixes on your site.<br /><br />
...
WPSmartContracts.com is not responsible for failed transactions, high fees, slow network response, or any other issue you might experience while interacting with the blockchain network.<br /><br />
...
WPSmartContracts.com commits to make every effort to ensure its technology platform is available for its users. Nonetheless, the customer accepts that the nature of the technology is to be under constant growth, numerous informatics incidences/issues, and continuous improvement; hence WPSmartContracts.com cannot commit to maintaining a 100% availability of its services, which the customer accepts and won’t hold WPSmartContracts.com responsible for the unavailability of such technological platform and/or any Blockchain platform, which administration is in no way the responsibility of WPSmartContracts.com.<br /><br />
...
TOS;
                ...
                $json = json_encode([
                    ...
                    'WRONG_RANGE_ROYALTIES' => __("Royalty should be a value between 0 and 100", 'wp-smart-contracts'),
                    'WRONG_RANGE_SUM' => __("The sum of commission plus royalty cannot be higher than 100", 'wp-smart-contracts'),
    ...
    public static function getTemplate($tempid) {
        ...
        if ( !file_exists( $template_path = get_stylesheet_directory() . '/wpsc-views/' . $template ) ) {
            if (!$template_path = self::isSkinFile($template)) { 
    ...
    static private function isSkinFile($template) {
        if ( $skin_path = self::getThemePath() and file_exists( $skin_file = $skin_path . '/wpsc-views/' . $template ) ) {
            return $skin_file;
        }
    }
